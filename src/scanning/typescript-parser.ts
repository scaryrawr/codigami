...
import { type CodeUnit, type LanguageParser, makeUnitId } from "../types.ts";
  ...
  switch (node.type) {
    ...
    case "export_statement": {
      ...
      if (declaration) {
        ...
        // export wrapping a lexical_declaration without field name
    ...
    async parse(filePath: string, source: string): Promise<CodeUnit[]> {
      ...
      for (const child of root.namedChildren) {
        if (EXTRACTABLE_TOP_LEVEL.has(child.type)) {
          units.push(...extractNode(child, filePath, lang));
        }
      }
